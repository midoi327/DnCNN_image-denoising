...
import math
import cv2
from scipy.ndimage import zoom # scipy.misc.imresize 함수 사용중단
...
def _get_patches_generic(img, patch_size, is_train, stride):
    ...
    scale_factor = 0.5
    img2 = cv2.resize(img, None, fx=scale_factor, fy=scale_factor, interpolation=cv2.INTER_CUBIC)
...
if __name__ == "__main__":
    ...
    for image_path in image_paths:
        ...
        # 왜곡된 이미지 (bikes_distorted.png)
        dis = np.array(Image.open(image_path).convert('L'))
