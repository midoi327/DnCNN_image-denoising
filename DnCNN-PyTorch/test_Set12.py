...
def main():
    ...
    files_source = glob.glob(os.path.join('data', opt.test_data, '*.png')) # 테스트하려는 이미지 포맷으로 수정 
    files_source.sort()
    ...
    for f in files_source:
        ...
        with torch.no_grad(): # this can save much memory
            Out = torch.clamp(ISource-model(ISource), 0., 1.)
        ...
        before_image = (ISource.cpu().numpy()[0,0]*255).astype(np.uint8)
